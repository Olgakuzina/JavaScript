function f(){
document.getElementById("pic1").src="2.gif";
document.getElementById("body1").style.backgroundColor="blue";
document.getElementById("body1").style.backgroundColor="yellow";
}
function g(){if(document.getElementById("sl1").selectedIndex==0)
document.getElementById("body1").style.backgroundColor="green";
if(document.getElementById("sl1").selectedIndex==1)document.getElementById("body1").style.backgroundColor="red";
if(document.getElementById("sl1").selectedIndex==2)
document.getElementById("body1").style.backgroundColor="pink";}
function rr(){document.getElementById("area1").value="Привет, Мир!!!";}